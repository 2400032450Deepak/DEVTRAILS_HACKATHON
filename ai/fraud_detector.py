...
# Add this function to your existing fraud_detector.py
...
def detect_sensor_fraud(sensor_data):
    ...
    # Real outdoor exposure vs AC room spoofing
    ...
    # Layer 9: Motion variance analysis (detect fake GPS walking patterns)
...
# Update the existing predict_fraud function to include sensor data
def predict_fraud(model, data):
    """
    ...
    # Rule 6: No motion detected (NEW - from sensor)
    if not has_motion and motion_confidence < 30:
        is_fraud = True
        reasons.append("No motion detected - possible GPS spoofing")
        anomaly_score += 0.25
    
    # Rule 7: Charging during high temperature (NEW)
    if is_charging and not has_motion and external_temp > 38:
        is_fraud = True
